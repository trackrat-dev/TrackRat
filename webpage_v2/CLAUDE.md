...
- **Manual**: `./scripts/deploy-webpage.sh [staging|production] [--cloudflare-only] [--dry-run]` from repo root (defaults to production; needs `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID`). A bare `production` run is refused while `trackrat.net` is still served from GCS and the `production` Worker has no routes — it would update an unserved target — so that window needs `--cloudflare-only`; push to the `production` branch to ship to the live site. The guard reads the pipeline's gsutil `sync` step, so deleting that at cutover retires it
