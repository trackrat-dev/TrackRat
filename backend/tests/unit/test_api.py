...
class TestContextPredictionAPI:
    ...
    def test_validation_allows_from_station_code_only(self, test_client, mock_train_repository, mock_train_stop_repository):
        ...
        # Mock context prediction data (would be generated by MP model)
        mock_prediction = MagicMock()
        mock_prediction.track_probabilities = {"4": 0.85, "3": 0.15}  # MP tracks, not NY tracks
    ...
    @patch('trackcast.api.routers.trains._enrich_with_context_prediction')
    def test_context_prediction_enrichment_called(self, mock_enrich_context, test_client, mock_train_repository, mock_train_stop_repository):
        ...
        mock_train_repository.get_trains.return_value = ([mock_train], 1)
        mock_train_stop_repository.get_stops_for_train.return_value = []
        mock_enrich_context.return_value = mock_train  # Return the same train

        # Test API call with from_station_code
        response = test_client.get("/api/trains?from_station_code=MP")
        
        # Verify context prediction enrichment was called
        assert response.status_code == 200
        mock_enrich_context.assert_called_once()
        # Verify it was called with the correct station code
        call_args = mock_enrich_context.call_args
        assert call_args[0][1] == "MP"  # Second argument should be the station code

    @patch('trackcast.api.routers.trains._enrich_with_context_prediction')
    def test_context_prediction_enrichment_not_called_without_station(self, mock_enrich_context, test_client, mock_train_repository, mock_train_stop_repository):
        """Test that context prediction enrichment is NOT called when from_station_code is not provided."""
        # Create mock train
        mock_train = MagicMock(spec=Train)
        mock_train.id = 1
        mock_train.train_id = "3829"
        ...
        # Verify context prediction enrichment was NOT called
        assert response.status_code == 200
        mock_enrich_context.assert_not_called()

    def test_ios_context_prediction_scenario(self, test_client, mock_train_repository, mock_train_stop_repository):
        """Test the specific iOS scenario: train details with from_station_code for context predictions."""
        # Create mock train that originated from NY but user is boarding at MP
        mock_train = MagicMock(spec=Train)
        mock_train.id = 1
        mock_train.train_id = "3765"
        mock_train.line = "Northeast Corrdr"
        ...
        # Mock context prediction for MP (would show different tracks than NY)
        mock_prediction = MagicMock()
        mock_prediction.track_probabilities = {"4": 0.8, "3": 0.2}  # MP tracks, not NY tracks (13, 14)
        mock_prediction.prediction_factors = [{"feature": "destination", "importance": 0.5, "direction": "positive", "explanation": "Princeton Junction trains typically use track 4 at Metropark"}]
        mock_prediction.model_version = "1.0.0_MP"  # Metropark model, not NY model
        mock_prediction.created_at = datetime.fromisoformat("2025-05-09T14:20:00")
        mock_train.prediction_data = mock_prediction
        ...
        # Simulate iOS API call: train details with context
        ...
        # Verify we get the train with context predictions
        train = data["trains"][0]
        assert train["train_id"] == "3765"
        assert train["origin_station_code"] == "NY"  # Train still originated from NY
        
        # Verify context predictions (MP model, not NY model)
        assert train["prediction_data"] is not None
        assert train["prediction_data"]["model_version"] == "1.0.0_MP"
        assert "4" in train["prediction_data"]["track_probabilities"]  # MP track
        assert "13" not in train["prediction_data"]["track_probabilities"]  # Not NY track
