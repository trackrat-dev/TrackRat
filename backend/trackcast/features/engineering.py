...
def load_track_data_from_csv(data_dir: str = "output/processed_data") -> Tuple[pd.DataFrame, List[str], List[str], List[str]]:
    """
...
def build_track_distributions(df: pd.DataFrame, tracks: List[str]) -> Tuple[Dict, Dict, Dict, Dict, Dict, Dict]:
    """
    ...
    logger.info(f"Built distributions for {len(train_id_dist)} train IDs, {len(line_dist)} lines, {len(dest_dist)} destinations")
...
def prepare_model_data_from_csv(df: pd.DataFrame, tracks: List[str], feature_cols: List[str], 
                               output_dir: str = "output") -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, int, int, List[str], Optional[np.ndarray], StandardScaler]:
    """
    ...
    numerical_cols = [col for col in feature_cols if col not in categorical_cols and not col.startswith('Line_')]
    numerical_cols = [col for col in numerical_cols if col in df.columns]
    
    if not numerical_cols:
        raise ValueError("No valid numerical features found")
    
    logger.info(f"Processing {len(numerical_cols)} numerical features and {len(categorical_cols)} categorical features.")
    ...
    logger.info(f"Saved intermediate training data to {output_path / 'intermediate_training_data.csv'}")
