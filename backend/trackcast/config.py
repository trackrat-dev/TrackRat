...
class Settings:
    ...
    def _set_defaults(self) -> None:
        ...
        if database_url:
            ...
            self._settings.database.url = database_url
            logger.info(f"Using DATABASE_URL from environment variable")
        elif not self._settings.database or not getattr(self._settings.database, "url", None):
            # Fallback to SQLite if no YAML config exists
            if not self._settings.database:
                self._settings.database = DotDict({})
