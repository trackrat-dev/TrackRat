...
class TrackPredictionPipeline:
    ...
    def predict(self, features):
        ...
        try:
            ...
            return {track: uniform_prob for track in self.track_to_idx.keys()} or {
                "1": 0.25,
                "2": 0.25,
                "3": 0.25,
                "4": 0.25,
    ...
    def get_prediction_factors(
        self, features: Dict[str, Any], use_perturbation=True
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            with torch.no_grad():
                baseline_pred = self.model(
                    feature_tensor, apply_softmax=True, apply_temperature=True
                )
            ...
            for feat_idx in range(num_features):
                ...
                perturbed_tensor = torch.tensor(
                    perturbed_sample.reshape(1, -1), dtype=torch.float32
                )
                with torch.no_grad():
                    perturbed_pred = self.model(
                        perturbed_tensor, apply_softmax=True, apply_temperature=True
                    )
                ...
                factors.append(
                    {
                        ...
                        "explanation": self._get_factor_explanation(
                            feature_name, importance, predicted_track_idx
                        ),
                    }
                )
    ...
    def _get_heuristic_factors(self) -> List[Dict[str, Any]]:
        ...
        time_features = [
            ...
            if any(x in col.lower() for x in ["hour", "day", "weekend", "rush"])
        ]
        if time_features:
            factors.append(
                {
        ...
        util_features = [
            col for col in self.feature_columns if "Utilization" in col or "Occupied" in col
        ]
        if util_features:
            factors.append(
                {
    ...
    def _get_factor_explanation(
        self, feature_name: str, importance: float, predicted_track_idx: int
    ) -> str:
    ...
    def _save_pytorch_model(
        self, encoders=None, target_encoder=None, tracks=None, custom_path=None
    ):
        ...
        if custom_path:
            ...
            if "_" in version:
                ...
                if len(parts) == 2 and parts[1].isupper() and len(parts[1]) <= 3:
                    ...
                    model_path = (
                        model_dir / f"track_pred_model_{base_version}_{station_code}_{timestamp}.pt"
                    )
                else:
        ...
        metadata = {
            ...
            "hidden_dims": [
                self.model.features[0].out_features
            ],  # Simplified - just first hidden layer
            ...
            "temperature": (
                float(self.model.temperature.item()) if hasattr(self.model, "temperature") else 1.0
            ),
        }
        ...
        try:
            ...
            if csv_mode:
                ...
                (
                    ...
                    class_weights,
                    scaler,
                ) = prepare_model_data_from_csv(df, tracks, feature_cols)
                ...
                logger.info(
                    f"CSV mode: Loaded {X_train.shape[0]} training and {X_test.shape[0]} test samples"
                )
            ...
            if val_loader is not None:
                ...
                try:
                    ...
                    logger.info(
                        f"Temperature calibration completed. Final temperature: {temperature:.4f}"
                    )
                except Exception as e:
            ...
            return {
                ...
                "val_samples": (
                    len(val_data)
                    if val_data and not csv_mode
                    else (X_val_scaled.shape[0] if X_val_scaled is not None else 0)
                ),
                "epochs": num_epochs,
    ...
    def _generate_prediction_summary(
        self, predicted_track: str, confidence: float, top_factors: List[Dict[str, Any]]
    ) -> str:
    ...
    def analyze_prediction_batch(
        self, features_list: List[Dict[str, Any]], include_explanations=True
    ) -> Dict[str, Any]:
        """
        ...
        for i, features in enumerate(features_list):
            try:
                ...
                predictions.append(
                    {"index": i, "predicted_track": "error", "confidence": 0.0, "error": str(e)}
                )
    ...
    def _generate_batch_summary(
        self, total: int, avg_confidence: float, distribution: Dict[str, int]
    ) -> str:
    ...
    def get_model_statistics(self) -> Dict[str, Any]:
        ...
        if self.model:
            ...
            stats["model_architecture"] = {
                ...
                "temperature": (
                    float(self.model.temperature.item())
            ...
            stats["parameters"] = {"total": total_params, "trainable": trainable_params}
    ...
    def validate_features(self, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        validation = {"valid": True, "warnings": [], "errors": [], "feature_info": {}}
        ...
        if extra_features:
            validation["warnings"].append(
                f"Extra {len(extra_features)} features will be ignored: {extra_features[:5]}..."
            )
