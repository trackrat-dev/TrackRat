...
def plot_confusion_matrix(
    ...
    labels = sorted(
        set(y_true) | set(y_pred), key=lambda x: int(x) if x.isdigit() else float("inf")
    )
...
def plot_all_confusion_matrices(
    ...
    overall_files = plot_confusion_matrix(
        y_true, y_pred, output_dir, model_version, timestamp, normalize=True, show_plot=show_plot
    )
    ...
    for line in unique_lines:
        ...
        if len(line_indices) < min_samples:
            logger.info(
                f"Skipping confusion matrix for line '{line}': insufficient samples ({len(line_indices)})"
            )
            continue
        ...
        logger.info(
            f"Generating normalized confusion matrix for line '{line}' with {len(line_indices)} samples"
        )
    ...
    for dest in unique_destinations:
        ...
        if len(dest_indices) < min_samples:
            logger.info(
                f"Skipping confusion matrix for destination '{dest}': insufficient samples ({len(dest_indices)})"
            )
            continue
        ...
        # Generate normalized confusion matrix
        logger.info(
            f"Generating normalized confusion matrix for destination '{dest}' with {len(dest_indices)} samples"
        )
...
def plot_confusion_matrix_comparison(
    ...
    for group in unique_groups:
        ...
        if len(group_indices) < min_samples:
            logger.info(
                f"Skipping {group_key} '{group}' in comparison: insufficient samples ({len(group_indices)})"
            )
            continue
