...
def plot_performance_by_time(
    ...
    if not hours_with_data.empty:
        error = 1.96 * np.sqrt(
            hours_with_data["accuracy"]
            * (1 - hours_with_data["accuracy"])
            / hours_with_data["count"]
        )

        ax.errorbar(
            hours_with_data["hour"],
            hours_with_data["accuracy"],
            yerr=error,
            fmt="o-",
            markersize=8,
    ...
    if not hours_no_data.empty:
        for i, row in hours_no_data.iterrows():
            ax.annotate(
                "No Data", (row["hour"], 0.02), ha="center", va="bottom", fontsize=8, alpha=0.6
            )
    ...
    error_with_data = 1.96 * np.sqrt(
        days_with_data["accuracy"] * (1 - days_with_data["accuracy"]) / days_with_data["count"]
    )
    ...
    bars = ax.bar(x_pos, day_perf["accuracy"].fillna(0), color=COLORS["primary"], alpha=0.7)

    # Add error bars only for days with data
    if not days_with_data.empty:
        days_with_data_indices = days_with_data.index
        ax.errorbar(
            days_with_data_indices,
            days_with_data["accuracy"],
            yerr=error_with_data,
            fmt="none",
            capsize=5,
            ecolor=COLORS["secondary"],
        )
    ...
    for i, row in day_perf.iterrows():
        if pd.notna(row["accuracy"]):
            ...
            ax.annotate("No Data", (i, 0.02), ha="center", va="bottom", fontsize=9, alpha=0.6)
    ...
    combined_perf = (
        df.groupby(["day_of_week", "hour"])["is_correct"].agg(["count", "mean"]).reset_index()
    )
    combined_perf.columns = ["day_of_week", "hour", "count", "accuracy"]

    # Create a pivot table for the heatmap
    pivot_accuracy = combined_perf.pivot_table(
        values="accuracy", index="day_of_week", columns="hour", fill_value=np.nan
    )

    # Create a pivot table for the counts
    pivot_count = combined_perf.pivot_table(
        values="count", index="day_of_week", columns="hour", fill_value=0
    )
    ...
    # Set titles and labels
    ax.set_title(
        f"Model Accuracy by Day and Hour (v{model_version})\nBlank cells have fewer than 5 samples"
    )
...
def plot_track_usage_heatmap(
    ...
    # Create dataframe for analysis
    df = pd.DataFrame({"track": tracks, "timestamp": timestamps})
