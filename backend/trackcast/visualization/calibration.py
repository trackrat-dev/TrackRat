...
def plot_calibration_curve(
    ...
    fig, ax, ece = plot_comprehensive_calibration(
        np.array(confidences), correctness, title=title, n_bins=n_bins, figsize=figsize
    )
...
def plot_calibration_analysis(
    ...
    group_title = f" - {group_name} {group_key}" if group_key and group_name else ""
    ax_main.set_title(
        f"Track Prediction Calibration{group_title}", fontsize=14, fontweight="bold", pad=20
    )
    ...
    with open(summary_file, "w") as f:
        ...
        f.write("=" * 70 + "\n\n")
        f.write(
            f"Generated: {timestamp if timestamp else datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
        )
...
def plot_all_calibration_curves(
    ...
    for line in unique_lines:
        ...
        if len(line_indices) < min_samples:
            logger.info(
                f"Skipping calibration analysis for line '{line}': insufficient samples ({len(line_indices)})"
            )
        ...
        logger.info(
            f"Generating calibration analysis for line '{line}' with {len(line_indices)} samples"
        )
        line_files = plot_calibration_analysis(
            ...
            group_key="line",
            group_name=line,
            destinations=line_destinations,
            show_plot=show_plot,
        )
    ...
    for dest in unique_destinations:
        ...
        if len(dest_indices) < min_samples:
            logger.info(
                f"Skipping calibration analysis for destination '{dest}': insufficient samples ({len(dest_indices)})"
            )
            continue
        ...
        logger.info(
            f"Generating calibration analysis for destination '{dest}' with {len(dest_indices)} samples"
        )
        ...
        for line, files in saved_files.get("lines", {}).items():
            if "summary" in files:
                with open(files["summary"], "r") as sf:
                    ...
                    match = re.search(
                        r"Expected Calibration Error \(ECE\): ([\d.]+)", summary_content
                    )
                    if match:
