...
def setup_plot(figsize: Tuple[int, int] = (10, 6), style: str = 'whitegrid') -> Tuple[plt.Figure, plt.Axes]:
    """
...
def setup_grid(grid_layout: Tuple[int, int] = (2, 2), 
               figsize: Tuple[int, int] = (15, 12),
               height_ratios: Optional[List[int]] = None,
               width_ratios: Optional[List[int]] = None) -> Tuple[plt.Figure, List[plt.Axes]]:
    """
...
def calculate_ece(confidences: np.ndarray, correctness: np.ndarray, n_bins: int = 10) -> Tuple[float, np.ndarray, np.ndarray, np.ndarray]:
    """
...
def add_value_labels(ax: plt.Axes, values: List, fmt: str = '{:.3f}', offset: Tuple[float, float] = (0, 5)):
    """
