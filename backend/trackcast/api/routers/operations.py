...
async def get_scheduler_auth(request: Request):
    ...
    if not verify_scheduler_auth(request):
        raise HTTPException(
            status_code=401,
            detail="Unauthorized: Valid scheduler authentication required"
        )
...
async def trigger_data_collection(
    ...
    _auth: None = Depends(get_scheduler_auth)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if success:
            logger.info(f"Data collection completed successfully in {processing_time:.3f}s: {stats}")
...
async def trigger_feature_processing(
    ...
    try:
        ...
        if success:
            logger.info(f"Feature processing completed successfully in {processing_time:.3f}s: {stats}")
            return response_data
        else:
...
async def trigger_prediction_generation(
    ...
    try:
        ...
        if success:
            logger.info(f"Prediction generation completed successfully in {processing_time:.3f}s: {stats}")
            return response_data
        else:
