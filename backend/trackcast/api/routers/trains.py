...
def _matches_target_station(stop, stops_at_station, stops_at_station_code, stops_at_station_name):
    ...
    if stops_at_station:
        ...
        return (stop.station_name and 
                stops_at_station_name.lower() in stop.station_name.lower())
    return False


def _filter_trains_by_stop_order(trains, origin_station_code, stops_at_station, stops_at_station_code, stops_at_station_name):
    """Filter trains where origin station comes before target station in route and both stops haven't departed."""
    if not origin_station_code or not any([stops_at_station, stops_at_station_code, stops_at_station_name]):
        return trains
    ...
    for train in trains:
        ...
        for i, stop in enumerate(train.stops):
            ...
            if _matches_target_station(stop, stops_at_station, stops_at_station_code, stops_at_station_name):
                target_index = i
...
async def list_trains(
    ...
    train_split: Optional[str] = Query(None, description="Filter by data split (train, validation, test)"),
    exclude_train_split: Optional[str] = Query(None, description="Exclude trains with this data split"),
    sort_by: Optional[str] = Query(None, description="Field to sort by (e.g., departure_time, line, destination, status)"),
    ...
    consolidate: bool = Query(
        False, description="Consolidate duplicate trains from multiple sources into unified journeys"
    ),
    ...
    try:
        ...
        if from_station_code and to_station_code and from_station_code == to_station_code:
            raise HTTPException(
                status_code=400, 
                detail="from_station_code and to_station_code cannot be the same"
            )
        ...
        if to_station_code:
            if not station_mapper.is_valid_code(to_station_code):
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid to_station_code: '{to_station_code}'"
                )
            # Translate frontend code to database code for query
        ...
        if origin_station_code:
            if not station_mapper.is_valid_code(origin_station_code):
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid origin_station_code: '{origin_station_code}'"
                )
            # Translate frontend code to database code for query
        ...
        if stops_at_station_code:
            ...
            # Translate frontend code to database code for query
            stops_at_station_code = station_mapper.translate_frontend_to_db_code(stops_at_station_code)
        ...
        if consolidate:
            ...
            for i, journey in enumerate(consolidated_trains):
                logger.info(f"  Journey {i+1}: {journey['train_id']} with {len(journey['data_sources'])} sources")
...
async def get_train(
    ...
    try:
        ...
        if train_id.isdigit():
            ...
            if not train:
                raise HTTPException(status_code=404, detail=f"Train with database ID {db_id} not found")
        else:
