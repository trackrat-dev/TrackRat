...
- **`consolidate_api_lb`** (default `true`): tears down this workspace's dedicated API frontend (IP, url map, proxies, forwarding rules). This was production's Phase-4 step, when `apiv2.trackrat.net` was host-routed by the consolidated **webpage** LB (`terraform-webpage/`) to the API backend service. That webpage LB has since been deleted (2026-08-08) and production moved on to the tunnel, so the flag's lasting effect is the teardown, not the host-routing. **`terraform-webpage/main.tf` still declares the deleted LB unconditionally** (`webpage_production_backend`, `webpage_production`, `webpage_production_cert`, `webpage_production_ip`, the two proxies and both forwarding rules — no `count`/`for_each` gate), so a `terraform apply` in that root will plan to *recreate* it, with its routing and its cost. Runbook Phase P6 is the step that removes those blocks and has not been done. Do not apply `terraform-webpage/` until it is. HSTS with `preload` used to come from the LB's `custom_response_headers` and now comes from `webpage_v2/public/_headers`. Runbook: `RUNBOOK-lb-consolidation.md`.
...
Worker names, custom domains and the SPA fallback live in `webpage_v2/wrangler.jsonc`; serving policy (cache headers, HSTS, the AASA `application/json` content type) lives in `webpage_v2/public/_headers`. Neither is in the pipeline. There is deliberately **no** `_redirects` file — `assets.not_found_handling` already covers deep links, and a catch-all would shadow real assets. `webpage_v2/vite.config.test.ts` pins the header policy against a real build.

The site is assets-only (no `main` in `wrangler.jsonc`), so no Worker script runs and no request is billed as an invocation.

Deploys need two secrets, read by both triggers from Secret Manager:
`cloudflare-workers-api-token` (*Workers Scripts: Edit* on the account, plus *Workers Routes: Edit* on the trackrat.net zone for the custom domains) and `cloudflare-account-id`. The Cloud Build service account (`trackrat-staging@trackrat-v2.iam.gserviceaccount.com`, used by both webpage triggers) needs `roles/secretmanager.secretAccessor` on each.
...
├── terraform-webpage/
│   └── main.tf                  # Webpage Cloud Build triggers + GCS buckets (rollback artifacts only). Still declares the LB / SSL cert / IP / forwarding rules that were deleted out of band 2026-08-08, so an apply here would recreate them — see the warning under "Frontend Topology"
└── terraform/
