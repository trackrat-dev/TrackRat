...
- **APIs** (`apiv2`, `staging.apiv2`) → **Cloudflare Tunnel** (`cloudflared`
  ...
  (plain HTTP on `:8000`).
- **Static site** (`trackrat.net`, `www`) → **Cloudflare Workers Static
  Assets**, replacing the GCS buckets. The original design here — front the
  bucket with Cloudflare via host/path transform rules — **is not implementable
  on our plan** and was withdrawn: overriding the `Host` header (required so
  GCS routes to the right bucket) is Enterprise-only on Origin Rules, and URL
  Rewrite explicitly "cannot rewrite the hostname". See issue #1713.
...
### S8. Rehearse Phase 3 — `staging.trackrat.net` on Workers Static Assets
...
1. Create an API token with **Workers Scripts: Edit** (account) and **Workers
   Routes: Edit** on the `trackrat.net` zone. The zone permission is what lets
   `wrangler deploy` attach the custom domain declared in `wrangler.jsonc`.
   Note the account ID.
2. Store both in Secret Manager and grant the Cloud Build service account
...
```bash
printf '%s' '<TOKEN>' | gcloud secrets create cloudflare-workers-api-token \
  --project=trackrat-v2 --replication-policy=automatic --data-file=-
printf '%s' '<ACCOUNT_ID>' | gcloud secrets create cloudflare-account-id \
  --project=trackrat-v2 --replication-policy=automatic --data-file=-

for s in cloudflare-workers-api-token cloudflare-account-id; do
...
3. Confirm no CNAME record already exists on `staging.trackrat.net` — a Custom
   Domain cannot be created over one. (It is currently NXDOMAIN, so there is
   nothing to remove.)
...
`wrangler deploy --env staging`, which uploads the assets *and* creates the
`staging.trackrat.net` Custom Domain plus its DNS record and certificate,
because both are declared in `webpage_v2/wrangler.jsonc`.
...
curl -sS -o /dev/null -w '%{http_code}\n' $BASE/trains/TR/NY          # 200 (SPA fallback)
curl -sS -o /dev/null -w '%{http_code}\n' $BASE/train/3515/TR/NY      # 200 (shared-link shape)
...
Unlike the Pages alternative, the deep-link responses above are **documented**
to be 200: `assets.not_found_handling: "single-page-application"` serves
`/index.html` "with a `200 OK` status" for any request that matches no file.

One behaviour still worth confirming by hand, because it is the failure mode
with no error message: **deploy twice and re-check the custom domain.** The
route is declared in `wrangler.jsonc`, so a second `wrangler deploy` should be
idempotent — but a route silently dropped on redeploy would take
`staging.trackrat.net` down on an ordinary push.
...
is therefore forward-only: fix and redeploy. This is acceptable precisely
because staging is already dark.
...
Same shape as S8 against the `production` environment, but production is live,
so the order below matters. `infra_v2/cloudbuild-webpage.yaml` deploys to
**both** GCS and the Worker on purpose: it removes the window where one
destination is serving while the other goes stale. Do not delete the GCS steps
early.

1. Merge `main` → `production` so the dual-deploy pipeline is live. The
   `production` env in `wrangler.jsonc` has **no `routes`** yet, so this
   publishes the Worker to its `workers.dev` URL and touches no DNS.
...
# Static site on Workers: the shell, a deep link, and — the one that catches a
