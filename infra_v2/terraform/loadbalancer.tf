...
# NOTE: staging.trackrat.net used to be host-routed off this LB to a CDN-backed
# backend bucket wrapping gs://trackrat-webpage-staging. That path was gated on
# local.create_api_frontend, which is permanently false now that
# frontend_via_cloudflare defaults to true, and the webpage moved to Cloudflare
# (issue #1713). The cert, backend bucket, host rule and SNI attachment were
# removed rather than left at count = 0.
...
# URL map. Every host on this LB routes to the API backend.
