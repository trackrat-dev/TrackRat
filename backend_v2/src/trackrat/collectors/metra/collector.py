...
# work when the upstream Metra endpoint is degraded.
...
class MetraCollector:
    ...
    async def collect(self, session: AsyncSession) -> dict[str, Any]:
        ...
        try:
            ...
            # Fetch all arrivals — let MetraFetchError propagate. Also wrap in
            # wait_for so a hung request (beyond the client's own timeout)
            ...
            try:
                ...
                logger.warning(
                    "metra_feed_fetch_timed_out | timeout_s=%.1f",
            ...
            if not merged_stops:
                ...
                if candidate_origin:
                    ...
                    origin_actual = synthetic_origin_departure(
                        first_arrival.arrival_time, now_for_provider(DATA_SOURCE)
                    )
                    if origin_actual:
