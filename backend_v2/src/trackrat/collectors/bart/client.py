"""
BART GTFS-RT client for fetching real-time train data.

Uses BART's official GTFS-RT feed directly with gtfs-realtime-bindings for parsing.
No API key required.
"""
...
class BartArrival(BaseModel):
    """A single arrival/stop time from the BART GTFS-RT feed."""

    station_code: str  # Our internal station code (e.g., BART_EMBR)
    gtfs_stop_id: str  # Original GTFS stop_id (e.g., M16-1)
    trip_id: str
    route_id: str
    ...
    delay_seconds: int
    track: str | None  # Always None for BART (fixed platforms)
...
class BARTClient:
    """
    Client for fetching BART real-time data from GTFS-RT feed.
    ...
    def __init__(self, timeout: float = 30.0) -> None:
        ...
        self._cache: list[BartArrival] | None = None
    ...
    def session(self) -> httpx.AsyncClient:
        ...
        if self._session is None:
            self._session = httpx.AsyncClient(
                ...
                headers={
                    "User-Agent": "TrackRat/2.0 (BART Real-time Tracker)",
    ...
    async def __aenter__(self) -> "BARTClient":
    ...
    def _get_route_info(self, route_id: str) -> tuple[str, str, str] | None:
        ...
        return BART_ROUTES.get(route_id)

    async def get_all_arrivals(self) -> list[BartArrival]:
        """
        Fetch and parse all BART arrivals from GTFS-RT feed.

        Returns cached data if still valid (within TTL).

        Returns:
            List of BartArrival objects for all upcoming stops
        """
        if self._is_cache_valid():
            return self._cache or []

        try:
            response = await self.session.get(BART_GTFS_RT_FEED_URL)
            response.raise_for_status()

            # Parse protobuf (BART returns valid protobuf despite text/html
            # Content-Type header)
            feed = gtfs_realtime_pb2.FeedMessage()
            feed.ParseFromString(response.content)

            arrivals: list[BartArrival] = []
                ...
                for stu in trip_update.stop_time_update:
                    ...
                    arrivals.append(
                        BartArrival(
                            ...
                            track=None,  # BART has fixed platforms
                        )
                    )
            ...
            logger.info(f"Fetched {len(arrivals)} BART arrivals from GTFS-RT feed")
            return arrivals

        except httpx.HTTPError as e:
            logger.error(f"HTTP error fetching BART GTFS-RT feed: {e}")
            # Return cached data if available, even if stale
            return self._cache or []
        except Exception as e:
            logger.error(f"Error parsing BART GTFS-RT feed: {e}")
            return self._cache or []

    async def get_trip_stops(self, trip_id: str) -> list[BartArrival]:
        """
