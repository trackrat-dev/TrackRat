"""
Metro-North unified collector for train discovery and journey updates.

Uses the MNRClient to fetch GTFS-RT data and creates/updates TrainJourney records.
...
def _generate_train_id(
    ...
    Format: M{trip_suffix}

    The "M" prefix ensures Metro-North trains are displayed as "M6317" etc.
    """
    ...
    return f"M{trip_suffix}"


class MNRCollector:
    """
    Unified Metro-North collector that discovers trains and updates journeys.
    ...
    Similar to LIRR's unified collector pattern.
    """

    def __init__(self, client: MNRClient | None = None) -> None:
        """Initialize collector.

        Args:
            client: Optional MNRClient instance. Creates new one if not provided.
        """
    ...
    async def collect(self, session: AsyncSession) -> dict[str, Any]:
        """
        Collect Metro-North train data.
        ...
        try:
            ...
            if not arrivals:
                logger.warning("No MNR arrivals found in GTFS-RT feed")
                return stats

            # Group arrivals by trip_id
            trips: dict[str, list[MnrArrival]] = {}
    ...
    async def _process_trip(
        ...
        # Get route info
        route_info = MNR_ROUTES.get(route_id)
        if route_info:
            line_code, line_name, line_color = route_info
        else:
            line_code = f"MNR-{route_id}"
            line_name = f"Metro-North Route {route_id}"
            line_color = "#0039A6"  # Default MNR blue
