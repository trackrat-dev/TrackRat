"""
Route congestion analysis service - database-aggregated calculation.

All congestion is computed by the optimized SQL queries built on the shared
``_stop_pairs_cte``. The divergent Python fallback that recomputed segments in
application code was removed in issue #1603; it had no runtime callers and had
drifted from the SQL path's window and cancellation semantics.
"""
