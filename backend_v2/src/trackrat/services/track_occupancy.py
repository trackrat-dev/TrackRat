...
class TrackOccupancyService:
    ...
    async def get_occupied_tracks(self, station_code: str) -> OccupiedTracksResponse:
        ...
        async with self._fetch_lock:
            ...
            cached_result = self._cache.get(cache_key)
            if cached_result:
                logger.debug("occupied_tracks_cache_hit_after_lock", station_code=station_code)
                return cached_result
    ...
    async def _get_database_tracks(self, station_code: str, session: AsyncSession) -> Set[str]:
