...
# Both accept `lines=MA,Ma` (comma-separated line codes, filtered server-side before
# the limit) so shared-terminal routes can be scoped to one line — same filter as
# /routes/history and /routes/summary?scope=route
...
- **Every 15 min**: NJT journey maintenance (silent-cancellation reconcile + old-journey expiry sweeps, re-homed from the dead `JourneyCollector.collect()` pipeline, #1497)
- **Every 2 min**: Legacy partition backfill (`journey_stops`/`segment_transit_times`; no-ops once the `*_legacy` tables are dropped — see "Partitioning")
...
- **Every 15 min**: Resource usage check (logs data-disk, boot-filesystem and database size for Cloud Monitoring alerting). Both filesystems are reported as separate events (`data_disk_usage_check` / `boot_disk_usage_check`) and therefore separate metrics — the alerts reduce with `REDUCE_MEAN`, so one shared series would let a healthy data disk average a full boot disk back under the threshold (issue #1826)
...
TRACKRAT_JOURNEY_UPDATE_BATCH_SIZE=100           # Max NJT journeys refreshed per periodic-update tick (oldest-updated-first)
...
TRACKRAT_ENABLE_METRICS=true                         # Mount the Prometheus /metrics endpoint
...
- ✅ Disk alerting can now see the filesystem that actually fills, and notices when it stops being able to see anything (issue #1826). On 2026-09-01 the VM's boot disk filled and stayed full for **19 days** with no alert, no autohealing, and `/health` reporting `healthy` throughout — three compounding reasons, none of them a threshold that needed tuning. (1) Every disk check pointed at `settings.data_disk_path`, the mounted persistent disk, and explicitly not at the boot filesystem (that was #1344's deliberate choice); the data disk was genuinely healthy the whole time, so `/metrics` reported `system_disk_usage_percent 57.5` while `/` sat at 100%. `check_resource_usage` now samples `BOOT_DISK_PATH` too and emits a **separate** `boot_disk_usage_check` event feeding its own metric and its own 75%/85% policies — separate because the alerts reduce with `REDUCE_MEAN`, and 100% averaged with 57% is 78.75%, under the page. (2) The alerts are all thresholds on **log-based** metrics, so when the full disk broke Docker's log writing the metrics stopped receiving points — and a threshold on a metric with no data does not fire, it goes quiet. The monitoring intended to catch disk exhaustion was disabled by the disk exhaustion. A new `condition_absent` policy ("Application Logs Absent", 1h) inverts that: no data *is* the alert, so it covers a full disk, a crashed scheduler, a wedged container or a broken logging agent without enumerating them. It reduces across series (absence is per-series and these are keyed by `instance_id`, so an unreduced policy would page forever about every instance the MIG has replaced) and sets `auto_close` to 604800s with a daily `renotify_interval`, rather than the threshold policies' 1800s. Note that *omitting* `auto_close` would not have meant "never": Cloud Monitoring applies the same 7-day default to an incident whose condition has stopped receiving data, which is every incident this policy can open — so a repeat of the 19-day outage would have closed itself at day 7 with the logs still absent. 7 days is the maximum, so re-notification rather than a longer incident is what covers a blackout beyond it. (3) MIG autohealing probes `/health/ready`, which checks Postgres and the scheduler, both on the healthy data disk — deliberately left alone, since a disk check there would make a full disk trigger VM recreation, masking the bug on a loop while discarding the logs each time. Structural invariants are pinned in `tests/unit/test_disk_alerting.py` against the real `.tf` files, including the event-name coupling between `metrics.tf` and `scheduler.py` that nothing else checks. Still open: a source of truth that doesn't depend on the app at all (Ops Agent disk metrics), and the fact that **staging has no alerting at all** — every policy is `var.environment == "production"`. (`services/scheduler.py`, `infra_v2/terraform/metrics.tf`, `infra_v2/terraform/monitoring.tf`)

### Recent Improvements (August 2026)
- ✅ Track predictions are no longer served for a station a train only *arrives* at (issue #1773). A rider whose selected route started at NY Penn opened NJT 7832 (Trenton → NY Penn) and the details screen showed Penn platform predictions; 7832 terminates at Penn and never boards there. Data scarcity does not suppress this on its own — it *causes* it. Providers publish tracks for departures, so a terminal stop carries no `track` of its own (confirmed across NJT arrivals into Penn, including completed runs), `_get_train_id_distribution` finds nothing, and `HistoricalTrackPredictor`'s hierarchy falls through to `_get_time_line_distribution`, which returns the track distribution of the *opposite-direction* trains on the same line scheduled around the same minute (production returned `historical_v1_time_line_code`, 93 records). The reach is wider than train-number search: every client sends the user's selected route origin as `from_station` for whatever train it opens, so any train terminating at the rider's home station hits this — NY, HB, TR, LB, DN, GCT and the rest of `STATION_PREDICTION_CONFIGS` are mostly terminals. Guarded once in `utils/train.journey_terminates_at_station` (delegating positional detection to `terminal_stop_index`, so a not-yet-sequenced discovery row still gets its prediction) and applied at both serving paths — the inline `track_prediction` on `/trains/{train_id}` and the standalone `/predictions/track`, which 404s with a reason naming the terminal arrival. Backend-only by design: iOS (including shipped builds) and web both hide the card on a missing/404 prediction, so the invariant stays single-sourced instead of being restated in Swift and TypeScript. (`utils/train.py`, `api/trains.py`, `api/predictions.py`)
- ✅ The NJT timetable is no longer written into `actual_departure` (issue #1768). A rider on train 7825 (NY→TR, 35 minutes late) reported that some stops showed 30+ minute delays while others showed none: six of its eleven departed stops carried an `actual_departure` exactly equal to their `scheduled_departure`, five of them recording a departure 37–46 minutes *before* the arrival recorded at that same stop. A schedule in the actuals column is indistinguishable from a train that ran on time, so those stops computed a zero delay everywhere at once — the train-detail row dropped its delay badge, `summary` counted the train as punctual, `_analyze_dwell_times` logged `invalid_dwell_time` and dropped the stop, and `_analyze_segments` measured the following hop from the scheduled time (a 5-minute hop recorded as ~40, passing the `<=0 or >300 min` validity guard and silently skewing `segment_transit_times`). Two writers produced it: `DepartureService._update_stops_from_embedded_data` assigned `stop.scheduled_arrival or stop.scheduled_departure` under a comment intending the live TIME estimate — `scheduled_arrival` is a different column from the live `updated_arrival` the same loop parses 30 lines earlier, and is almost always NULL for NJT — and NJT's tier 1/tier 2 departure inference fell back to `scheduled_departure` whenever the live estimate was not yet in the past, which for a delayed train is the normal case at the moment NJT flips `DEPARTED=YES`. (Tier 3 had always refused to do this, in a comment saying why; that fallback was introduced by #1221, whose real invariant — never a *future* timestamp — `None` satisfies.) Both writers now route through `utils/train.resolve_actual_departure`, which records only a live reading that has already happened, keeps the first capture frozen against NJT's hours-long revisions, and reopens that freeze for the one case that cannot be true — a departure stored before the arrival at the same stop — replacing it with the current reading or clearing it. That last clause is the repair path: both writers only ever filled a `NULL`, so a code-only fix would have left every already-corrupt row corrupt. **Rows on journeys no longer being refreshed are not repaired** and age out with the retention window; route-history/congestion baselines stay skewed for those until then. iOS carried the display half of the same anti-pattern — `TrainDetailsView`'s departed-stop branch fell from a missing `actualDeparture` straight to `scheduledDeparture` — and now uses the new `StopV2.bestKnownDeparture` (actual → live estimate → schedule), which also de-duplicates the identical chain inlined in `TrainV2.getDepartureTime`. (`utils/train.py`, `collectors/njt/journey.py`, `services/departure.py`, `ios/TrackRat/Models/TrainV2.swift`, `ios/TrackRat/Views/Screens/TrainDetailsView.swift`)
- ✅ PATH short-headway matching is now one-to-one in both phases (issue #1723). RidePATH would list a train due in 1–11 minutes with no TrackRat record at all, because both matching passes resolved each arrival *independently* against a ±5 minute window. On JSQ-33 / NWK-WTC (2–4 minute headways) consecutive trains share line, origin and destination, so every train in a run fell inside that window of the one journey already on record: discovery kept "finding" it and creating nothing (quantizing the line to ~one journey per 5 minutes), and the update pass wrote a single arrival onto the stops of every journey it fitted. The window cannot shrink — RidePATH reports whole-minute countdowns and the origin departure is back-calculated from GTFS segment times, so one train seen at two stations lands minutes apart, and a 2-minute window would just trade swallowed trains for duplicates. Instead each cycle groups sightings by (date, line, origin, destination), collapses repeat sightings of one physical train (same-station arrivals are never the same train), then pairs whole *runs* of trains at once via a shared `_match_in_order` primitive — used for both sighting→journey and arrival→stop. PATH trains on a line do not overtake, so that matcher is order-preserving, and it maximizes the number of pairs before minimizing total separation: ranking candidate pairs by distance and claiming greedily would take the single closest pair (10:03→10:04) and strand both neighbours, duplicating a train already tracked. Both tolerances now come from one `MATCH_TOLERANCE_MINUTES` constant, and a sighting whose station is not on the resolved route is dropped instead of getting a fabricated origin time. (`collectors/path/collector.py`)
- ✅ Connected SEPTA Metro's split station codes (issue #1709). Two gaps, both invisible to Source 5 because it only pairs lines meeting at ONE station code: **Broad St ↔ Market-Frankford** shared no code (they interchange across the City Hall / 15th St concourse), so every BSL↔MFL trip returned `no_transfer_points`; and every **subway-surface trolley tunnel platform** is its own code per direction — SEPTA_METRO is the only source with directional `reverse_stations` — so a rider reversing mid-tunnel had no interchange short of the 13th St terminal. New `SEPTA_METRO_STATION_COMPLEXES` (`config/stations/common.py`) drives a generalized Source 4, which now takes `(system, complexes, lines)` instead of hardcoding `"SUBWAY"`, and emits same-lines pairs where `route_topology.models_directions_separately(system)` — those are the cross-platform interchanges rather than redundant duplicates. Modeled as **transfers only**, deliberately *not* spread into `STATION_EQUIVALENCE_GROUPS` like `SUBWAY_STATION_COMPLEXES`: equivalence pools departure boards and alerts across the group, which for a directional platform pair would put outbound trolleys on the inbound board. `trip_search._orient_transfer` also had to stop trusting line overlap — both sides of a platform pair carry the same lines, so it now falls back to `_junction_legs_run` to pick the orientation whose legs actually run (a no-op for every non-directional system). Over 4000 random Metro pairs, pairs that can produce an itinerary go from 239 to 456, with no pair losing one. Transfer-modeled codes also need endpoint handling: `_cross_modal_hub_direct_trips` (#1587) generalized to `_station_complex_direct_trips`, since a complex code as a trip *endpoint* leaves the transfer with a zero-length first leg — City Hall's Broad St platform to a Market-Frankford stop resolves as one MFL ride from the sibling code instead of returning nothing (`search_type` metadata `cross_modal` → `station_complex`). (`config/stations/common.py`, `config/transfer_points.py`, `config/route_topology.py`, `services/trip_search.py`)

### Recent Improvements (July 2026)
- ✅ Fixed NJT cancellation-detection latency and the "100% on time" it produced (issue #1670). Train 3918 was annulled mid-journey and TrackRat showed it running for 65 minutes; the status bar read "Past two hours: 100% on time" the whole time and the 7am digest reported normal service. Three independent defects:
  - **`schedule_periodic_updates` starvation** (`services/scheduler.py`): the candidate query matched on lifecycle flags alone — satisfied by *every* NJT train of the day, since the nightly schedule collector sets `has_complete_journey=True` on all of them and journeys that never reach a terminal keep `is_completed=False` until the next day's expiry sweep — then applied `LIMIT 50` **before** a Python-side staleness filter, with no `ORDER BY`. Production logged `count=50 total_active_trains=50` on every 5-minute tick around the clock (including 4am), and which trains got refreshed came down to Postgres' physical row order; 3918 lost that lottery for 12 consecutive ticks and received no collection between the nightly schedule build and a user-triggered JIT refresh. Now: a `scheduled_departure` window (`ACTIVE_JOURNEY_LOOKBACK_HOURS`) restricts candidates to trains that can plausibly be in flight, staleness moved into SQL, batch ordered oldest-updated-first, size configurable via `TRACKRAT_JOURNEY_UPDATE_BATCH_SIZE` (default 100), and the true eligible count + backlog logged — the old `total_active_trains` was clamped by the limit and could never reveal a backlog.
  - **Cancellation-blind station-board refresh** (`services/departure.py`, `collectors/njt/discovery.py`): both paths ingest the board's `STOP_STATUS` values and stamp `last_updated_at` — marking the row fresh for the periodic sweep and the JIT staleness check — but never applied NJT's cancellation rule, so they actively *delayed* detection. The rule is now single-sourced in `utils/train.njt_cancellation_reason` (`njt_stops_indicate_cancellation` is its boolean form) and applied by `collect_journey_details`, the departure-board refresh, and discovery. These paths only ever *set* the flag; clearing a stale cancellation remains discovery's job (issue #1498).
  - **Stalled trains counted on time** (`services/summary.py`): `_calculate_departure_stats` computed `delay = estimate - scheduled`, so a train whose provider keeps republishing an unchanged on-time estimate scored zero delay forever — 3918's remaining stops all still read their original schedule. A not-yet-departed stop whose live estimate is more than `STALE_ESTIMATE_GRACE_MINUTES` in the past is now floored at wall clock, gated to `REAL_TIME_DATA_SOURCES` and `OBSERVED` journeys so schedule-first sources (PATCO, SEPTA Metro) and never-observed rows are unaffected.
  - Also: "1 cancellation" no longer renders above an unqualified "100% ... departing on time" — cancelled trains are excluded from the on-time denominator, so the population is now named ("100% of the 19 similar NJ Transit trains that ran departed on time") whenever a cancellation is reported alongside the percentage, and the sentence is dropped entirely when nothing ran.
