...
- ✅ A GTFS bundle whose service period has not started yet is **declined rather than adopted** (issue #1769), and a source dark for that reason is now visible on `/health` (issue #1770). Storing a bundle is destructive — `_parse_and_store_gtfs` clears the source's rows first — so an agency publishing next week's timetable early used to leave the source freshly downloaded, weeks from expiry, and serving nothing; SEPTA Regional Rail went dark for a day and a half that way while `/health` reported `healthy`. `refresh_feed` now decides *before* parsing via `_bundle_service_status` and returns `GTFSRefreshOutcome.SKIPPED_NOT_YET_ACTIVE` when the service has not begun and a usable bundle is stored; "has begun" is per-service (a retained `calendar.txt` row covering today, or a retained `calendar_dates.txt` addition dated today or earlier), honors `GTFS_ROUTE_TYPE_FILTER`, and an unknown window fails **open**. Two paths still adopt a future-dated bundle: a first-ever download with nothing stored (`gtfs_first_bundle_not_yet_active`) and a stored bundle that has itself lapsed (`gtfs_adopting_future_bundle_over_lapsed`). A decline stays out of `failed_sources` but is named in `declined_sources` on `gtfs_feed_refresh_complete`, and deliberately does not advance `last_successful_parse_at` — so a source publishing 3+ days early crosses `GTFS_STALE_FEED_HOURS` mid-wait and degrades `/health` until its start date. `/health`'s `gtfs_feeds` check reports `not_yet_active_sources` (with per-feed `feed_start_date` / `days_until_feed_start`) beside `lapsed_sources` and `stale_sources`; `scripts/verify-deployment.sh` asserts on both lists. Full operational reference in the root `CLAUDE.md`. (`services/gtfs.py`, `api/health.py`, `scripts/verify-deployment.sh`)
- ✅ `journey.actual_departure` is derived from the **origin stop** in every GTFS-RT collector (issue #1750). GTFS-RT prunes stops a trip has already passed, so the earliest stop still in the feed is not the trip's origin; seven collectors assigned that stop's time to `actual_departure`, pairing an origin `scheduled_departure` with a downstream actual — which every consumer that subtracts the two reads as departure delay that *grows* as the train progresses. `mta_common.origin_actual_departure()` existed for exactly this and had only been adopted by SUBWAY; it is now used at all three sites in each of the remaining seven collectors (discovery constructor, update branch, JIT refresh), with the assignment moved after `update_stop_departure_status()` so it reads the stops the cycle just wrote. `tests/unit/collectors/test_origin_departure_wiring.py` scans every `*.py` under `collectors/` for the raw substitution, so new collectors are covered automatically. (`collectors/{lirr,mnr,bart,mbta,metra,septa_rr,septa_metro}/collector.py`)
