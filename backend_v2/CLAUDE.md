...
- ✅ One physical station no longer appears several times in the station picker, each code serving half its trains. SEPTA Regional Rail runs over the NEC and Keystone Corridor and arrived carrying its own code for every station Amtrak and NJT already described, and none reached `STATION_EQUIVALENCE_GROUPS`: Philadelphia 30th Street Station existed as both "Philadelphia" (`PH`) and "Gray 30th St Station" (`SEPR90004`) 58 m apart, plus three `SEPM*` codes for "Drexel Station at 30th St"; Wilmington, Trenton and six Keystone stops were duplicated the same way. Picking the wrong one silently omitted the other provider's trains, and `transfer_points` Source 3 emitted the 58 m gap as a walking transfer between *different* stations (`same_station=False`). Ten SEPTA/Amtrak/NJT stations, NJT/PATCO Lindenwold, and the Amtrak/Metra + Amtrak/MBTA pairs are now grouped — the latter proactively, so clearing their `TRACKRAT_DISABLED_DATA_SOURCES` entry cannot reintroduce the bug. **Display identity is modeled separately from query identity** (`DISPLAY_STATION_GROUPS`): equivalence groups pool boards, but SEPTA's directional platform codes must not (see `SEPTA_METRO_STATION_COMPLEXES`), and yet all of them are one picker row. Clients dedupe *search results* by `display_station_code` rather than hiding non-representative codes, so every code keeps its own searchable name — subway complexes deliberately name each platform differently ("42 St-Port Authority Bus Terminal" and "Times Sq-42 St" are one station) and a canonical-name collapse would make the others unfindable. `CROSS_MODAL_HUBS` is deliberately **excluded**: 30th Street Station and the Drexel Station beneath it stay separately selectable, exactly as NY Penn and 34 St-Penn already do. The class of defect is now guarded rather than the instances — `test_nearby_rail_stations_are_grouped_or_allowlisted` fails when two rail providers publish stations within 250 m that are neither grouped nor listed in `DISTINCT_NEARBY_STATIONS`; the pre-existing `test_no_duplicate_coordinates` could never catch it, since it compares coordinates for *exact* equality and every feed publishes its own. Client mirrors are generated (`scripts/generate_display_station_keys.py`) and CI runs it with `--check`. (issue #1774, `config/stations/common.py`, `config/transfer_points.py`, `webpage_v2/src/data/stations.ts`, `ios/TrackRat/Shared/StationData.swift`, `ios/TrackRat/Shared/Stations.swift`)
  - ⚠️ Found while building the guard, **not fixed**: several pre-existing `SUBWAY_STATION_COMPLEXES` entries merge same-named stations that are not actually one complex — "125 St" spans 2163 m across four unconnected stations (1 at Broadway, 2/3 at Lenox, 4/5/6 at Lexington, A/B/C/D at St Nicholas), and "Gun Hill Rd" (1910 m), "7 Av" (1373 m), "145 St" (1369 m), "Pelham Pkwy" (1050 m) are the same shape. Those groups pool departure boards and alerts across genuinely different stations. Verifying each against MTA's real in-system transfers is its own change.
