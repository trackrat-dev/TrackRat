...
- ✅ Added server-side `statement_timeout=55s` (below the asyncpg `command_timeout=60s`) so Postgres cancels runaway queries itself instead of relying solely on the client-side cancel. Also fixed `get_operations_summary`'s `last_stops` subquery, which had no time-window filter and was sorting the entire `journey_stops` table on every request instead of just the journeys in the summary's cutoff window (issue #1366, `backend_v2/docker-compose.yml`, `services/summary.py`). `max_parallel_workers_per_gather` was deliberately left at `1` rather than `0` — that query genuinely needs parallelism for large scans until further query-level fixes land.
