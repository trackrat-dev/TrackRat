...
async def check_todays_problematic_train():
    from sqlalchemy.ext.asyncio import AsyncSession
    engine = get_engine()
    async with AsyncSession(engine) as session:
        # Find a train from today with stops_count=0 but has actual stops
        ...
        if journey:
            ...
            print('No trains found with stops_count=0')

if __name__ == "__main__":
    asyncio.run(check_todays_problematic_train())