...
class TestLirrArrival:
    """Tests for LirrArrival model."""

    def test_creation_with_all_fields(self):
        """Test creating LirrArrival with all fields."""
        arrival = LirrArrival(
            station_code="JAM",
            gtfs_stop_id="102",
            trip_id="trip_123",
...
class TestLIRRClient:
    """Tests for LIRRClient."""

    @pytest.fixture
    def client(self):
        """Create a LIRRClient instance for testing."""
        return LIRRClient(timeout=10.0)
    ...
    async def test_get_station_arrivals_filters_by_station(self, client):
        ...
        result = await client.get_station_arrivals("JAM")
    ...
    async def test_get_trip_stops_filters_by_trip(self, client):
        ...
        assert len(result) == 2
        assert result[0].station_code == "JAM"
        assert result[1].station_code == "NY"
    ...
    def test_map_stop_id_valid(self, client):
        ...
        # Jamaica station should map to JAM
        result = client._map_stop_id("102")
        assert result == "JAM"
    ...
    def test_get_route_info_valid(self, client):
        ...
        # Route 1 is Babylon Branch
        result = client._get_route_info("1")
        assert result is not None
        assert result[0] == "LIRR-BB"  # line_code
        assert result[1] == "Babylon Branch"  # name
