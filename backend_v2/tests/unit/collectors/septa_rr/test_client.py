...
class TestSeptaRailModels:
    ...
    def test_arrival_creation(self):
        arr = SeptaRailArrival(
            ...
            route_id="CHW",
            direction_id=0,
            arrival_time=datetime(2026, 7, 18, 10, 30, tzinfo=UTC),
            departure_time=datetime(2026, 7, 18, 10, 31, tzinfo=UTC),
            delay_seconds=120,
            track=None,
        )
...
class TestSeptaRailGetTripUpdates:
    ...
    async def test_batch_fetch_bypasses_valid_cache(self, client):
        ...
        assert await client.get_trip_updates(use_cache=False) == []
