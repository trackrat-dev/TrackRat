...
class TestFuzzyMatchScheduledTrain:
    ...
    @pytest.mark.asyncio
    async def test_reobservation_keeps_cancellation_when_still_cancelled(
        self, collector
    ):
        """A cancelled row that reappears in discovery while the live feed still
        marks it cancelled (terminal/all stops STOP_STATUS=CANCELLED) must NOT
        be un-cancelled. Discovery only copies times and bumps last_updated_at,
        which can make the next collection skip the row as fresh — so clearing
        here would show a still-cancelled train as running (PR #1519 review).
        """
        mock_session = _make_session_mock()
        ...
        existing_journey.is_expired = False
        existing_journey.is_cancelled = True
        existing_journey.cancellation_reason = "All stops cancelled by NJT"
        existing_journey.last_updated_at = None

        # Exact match → the cancelled existing journey.
        mock_session.scalar = AsyncMock(return_value=existing_journey)

        # STOPS carry STOP_STATUS but no TIME/DEP_TIME, so the stop-population
        # path is skipped while the cancellation gate still sees the statuses.
        train_data = [
            {
                "TRAIN_ID": "3965",
                "LINE": "NEC",
                ...
                "SCHED_DEP_DATE": "05-Jul-2025 10:15:00 AM",
                "STOPS": [
                    {"STATION_2CHAR": "NY", "STOP_STATUS": "CANCELLED"},
        ...
        assert result == set()
        # The still-confirmed cancellation is preserved.
