...
def generate_swift(
    ...
    lines = [
        ...
        "/// Segments without shape data should be drawn as straight lines.",
        "enum RouteShapes {",
        "",
        ...
        "        guard let points = canonicalCoordinates(forKey: key) else { return nil }",
        "        return fromStation > toStation ? points.reversed() : points",
        "    }",
        "",
        "    // MARK: - Shape Data",
        "    // Encoded as \"KEY|lat,lon,lat,lon,...\" text lines. Deliberately NOT a",
        "    // [String: [Double]] literal: a numeric collection literal this size",
        "    // (60k+ Doubles, compiled once per target) sends the Swift type checker",
        "    // into minutes-long territory and timed out iOS CI. String literals",
        "    // type-check in constant time.",
        "    //",
        "    // Decoding is per-segment, not all-or-nothing (issue #1626). `encodedByKey`",
        "    // indexes the text on first access but converts no coordinates; a lookup",
        "    // then parses only its own segment's numbers.",
        "",
        "    /// Coordinates in canonical (alphabetical) key order, decoded on demand.",
        "    private static func canonicalCoordinates(forKey key: String) -> [CLLocationCoordinate2D]? {",
        "        if let cached = decodedCache.object(forKey: key as NSString) { return cached.coordinates }",
        "        guard let encoded = encodedByKey[key], let points = decode(encoded) else { return nil }",
        "        decodedCache.setObject(DecodedSegment(points), forKey: key as NSString)",
        "        return points",
        "    }",
        "",
        "    /// Parse one \"lat,lon,lat,lon,...\" run. Returns nil for malformed records",
        "    /// (non-numeric field, odd count, or fewer than two points) so the caller",
        "    /// falls back to a straight line rather than drawing a partial polyline.",
        "    private static func decode(_ encoded: Substring) -> [CLLocationCoordinate2D]? {",
        "        var values: [Double] = []",
        "        for field in encoded.split(separator: \",\") {",
        ...
        "    private final class DecodedSegment {",
        "        let coordinates: [CLLocationCoordinate2D]",
        "        init(_ coordinates: [CLLocationCoordinate2D]) { self.coordinates = coordinates }",
        "    }",
        "",
        "    /// Bounded so redrawing a map can't accumulate a second, decoded copy of",
        "    /// the whole dataset alongside the string literal. NSCache is also",
        "    /// thread-safe, which a bare static dictionary would not be.",
        "    private static let decodedCache: NSCache<NSString, DecodedSegment> = {",
        "        let cache = NSCache<NSString, DecodedSegment>()",
        "        cache.countLimit = 128",
        "        return cache",
        "    }()",
        "",
        "    /// Segment key -> its encoded coordinate run. The values are `Substring`s",
        "    /// sharing the literal's storage, so the index costs no second copy of the",
        "    /// coordinate text and converts no Doubles.",
        f"    private static let encodedByKey: [String: Substring] = {{",
        f"        var result = [String: Substring](minimumCapacity: {total_segments})",
        "        for block in encodedShapeData {",
        '            for line in block.split(separator: "\\n") {',
        '                guard let separator = line.firstIndex(of: "|") else { continue }',
        "                result[String(line[..<separator])] = line[line.index(after: separator)...]",
        "            }",
        "        }",
        "        return result",
        "    }()",
