...
> **Note:** Any system can be disabled deployment-wide via the `TRACKRAT_DISABLED_DATA_SOURCES` env var (skips collection and filters API responses). The hosted TrackRat apps have BART, WMATA, MBTA, and Metra disabled; everything else, including SEPTA (Regional Rail + Metro), is live. Self-hosted deployments can enable any of them.
