...
| [google_cloud_scheduler_job.operations_jobs](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/cloud_scheduler_job) | resource |
...
|------|-------------|------|---------|:--------:|
| <a name="input_api_service_uri"></a> [api\_service\_uri](#input\_api\_service\_uri) | URI of the API service to target (instead of scheduler service) | `string` | `null` | no |
...
| <a name="input_scheduler_jobs"></a> [scheduler\_jobs](#input\_scheduler\_jobs) | Map of scheduler jobs to create with their configurations | <pre>map(object({<br/>    schedule    = string<br/>    endpoint    = string<br/>    description = string<br/>  }))</pre> | <pre>{<br/>  "data_collection": {<br/>    "description": "Collect train data from NJ Transit and Amtrak APIs",<br/>    "endpoint": "/api/ops/collect-data",<br/>    "schedule": "0 * * * *"<br/>  },<br/>  "feature_processing": {<br/>    "description": "Process features for collected train data",<br/>    "endpoint": "/api/ops/process-features",<br/>    "schedule": "10 * * * *"<br/>  },<br/>  "prediction_generation": {<br/>    "description": "Generate track predictions for upcoming trains",<br/>    "endpoint": "/api/ops/generate-predictions",<br/>    "schedule": "20 * * * *"<br/>  }<br/>}</pre> | no |
| <a name="input_scheduler_schedule"></a> [scheduler\_schedule](#input\_scheduler\_schedule) | Cron schedule for the legacy daily job (deprecated - use scheduler\_jobs instead) | `string` | `null` | no |
...
| <a name="input_scheduler_timezone"></a> [scheduler\_timezone](#input\_scheduler\_timezone) | Timezone for scheduler jobs | `string` | `"America/New_York"` | no |
...
| Name | Description |
|------|-------------|
| <a name="output_operations_job_names"></a> [operations\_job\_names](#output\_operations\_job\_names) | Names of the operations Cloud Scheduler jobs |
| <a name="output_operations_job_schedules"></a> [operations\_job\_schedules](#output\_operations\_job\_schedules) | Schedules of the operations Cloud Scheduler jobs |
...
| <a name="output_scheduler_job_name"></a> [scheduler\_job\_name](#output\_scheduler\_job\_name) | Name of the legacy Cloud Scheduler job |
